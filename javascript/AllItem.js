...
const subnavTitle = $(".header-subnav");
...
const displayProducts = (filteredProducts) => {
  productList.innerHTML = filteredProducts
    .map(
      (product, i) =>
        `        
                 <div class="product-item col-sm-4"   data-index-number=${i}>
...
subnavTitle.addEventListener("click", (e) => {
  const selectedCat = e.target.textContent;
  selectedCat === "Tất cả sản phẩm"
    ? displayProducts(perData)
    : displayProducts(perData.filter((item) => item.cat === selectedCat));
  categoryTitle.innerHTML = selectedCat;
  navTitle.innerHTML = selectedCat;
});
